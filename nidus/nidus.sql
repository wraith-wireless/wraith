...
CREATE TYPE ROLE AS ENUM ('abad','shama');
...
CREATE VIEW sessions AS
    SELECT s.session_id,s.hostname,lower(s.period) AS start,upper(s.period) AS stop,
           p.kernel,
           g.devid,
           (SELECT ur.mac FROM using_radio ur WHERE ur.sid = s.session_id AND ur.role = 'abad') as abad,
           (SELECT ur.mac FROM using_radio ur WHERE ur.sid = s.session_id AND ur.role = 'shama') as shama,
